...
def _trim_geometry(width_px:int,height_px:int,*,target_ppi:float,bleed_mm:float,trim_width_mm:float|None,trim_height_mm:float|None) -> dict:
    ...
    if trim_width_mm is None and trim_height_mm is None:
        trim_width_mm=width_px/target_ppi*25.4; trim_height_mm=height_px/target_ppi*25.4
        ...
        expected=trim_width_mm/trim_height_mm
        if abs(expected-aspect)/aspect>0.005: raise ValueError("Explicit trim dimensions must preserve source aspect ratio within 0.5%")
        basis="explicit_width_and_height"
    x_ppi=width_px/(float(trim_width_mm)/25.4); y_ppi=height_px/(float(trim_height_mm)/25.4)
    bleed_units_x=float(bleed_mm)/float(trim_width_mm)*width_px if bleed_mm else 0.0
    bleed_units_y=float(bleed_mm)/float(trim_height_mm)*height_px if bleed_mm else 0.0
    render_w=max(1,int(math.ceil(float(trim_width_mm)/25.4*target_ppi)))
    render_h=max(1,int(math.ceil(float(trim_height_mm)/25.4*target_ppi)))
    bleed_render_px=max(0,int(math.ceil(float(bleed_mm)/25.4*target_ppi)))
    return {
        ...
        "source_meets_target_ppi":min(x_ppi,y_ppi)+0.5>=target_ppi,
        "bleed_units_x":bleed_units_x,"bleed_units_y":bleed_units_y,
        "page_viewbox_width":width_px+2*bleed_units_x,"page_viewbox_height":height_px+2*bleed_units_y,
        "target_trim_render_width_px":render_w,"target_trim_render_height_px":render_h,"target_bleed_render_px":bleed_render_px,
...
def _write_physical_svg(root:ET.Element,path:Path,geometry:dict,width_px:int,height_px:int,*,bleed_data_uri:str|None=None) -> None:
    if bleed_data_uri is None:
        out=ET.Element(f"{{{SVG_NS}}}svg",{"width":f"{geometry['trim_width_mm']:.6f}mm","height":f"{geometry['trim_height_mm']:.6f}mm","viewBox":f"0 0 {width_px} {height_px}"})
        for child in root: out.append(deepcopy(child))
    else:
        bx=geometry["bleed_units_x"]; by=geometry["bleed_units_y"]; page_w=geometry["page_viewbox_width"]; page_h=geometry["page_viewbox_height"]
        out=ET.Element(f"{{{SVG_NS}}}svg",{"width":f"{geometry['page_width_mm']:.6f}mm","height":f"{geometry['page_height_mm']:.6f}mm","viewBox":f"0 0 {page_w:.6f} {page_h:.6f}"})
        ET.SubElement(out,f"{{{SVG_NS}}}image",{
            "id":"generated-bleed-extension","x":"0","y":"0","width":f"{page_w:.6f}","height":f"{page_h:.6f}","preserveAspectRatio":"none",
...
def _bleed_data_uri(master_svg:Path,inkscape:str,geometry:dict,temp:Path,commands:list[dict]) -> str:
    render=temp/"trim_target_ppi.png"; rw=geometry["target_trim_render_width_px"]; rh=geometry["target_trim_render_height_px"]
    ...
    rgb=np.asarray(Image.open(render).convert("RGB"),dtype=np.uint8); b=int(geometry["target_bleed_render_px"])
...
def _set_pdf_geometry_and_output_intent(source:Path,dest:Path,geometry:dict,icc:Path,profile_name:str) -> dict:
    bleed_pt=geometry["bleed_mm"]/25.4*72.0; page_w_pt=geometry["page_width_mm"]/25.4*72.0; page_h_pt=geometry["page_height_mm"]/25.4*72.0
    with pikepdf.open(source) as pdf:
        for page in pdf.pages:
            media=pikepdf.Array([0,0,page_w_pt,page_h_pt]); trim=pikepdf.Array([bleed_pt,bleed_pt,page_w_pt-bleed_pt,page_h_pt-bleed_pt])
...
def _name_count(obj,target:pikepdf.Name,seen:set[tuple[int,int]]) -> int:
    try:
        ...
        if isinstance(obj,pikepdf.Dictionary) or isinstance(obj,pikepdf.Stream): return sum(_name_count(v,target,seen) for _,v in obj.items())
...
def _pdf_structural_report(pdf_path:Path,geometry:dict,pdfimages:str|None,pdffonts:str|None) -> dict:
    with pikepdf.open(pdf_path) as pdf:
        page=pdf.pages[0]; boxes={name:[float(x) for x in page.obj.get(name,[])] for name in ("/MediaBox","/CropBox","/BleedBox","/TrimBox","/ArtBox")}
        rgb_names=sum(_name_count(obj,pikepdf.Name("/DeviceRGB"),set()) for obj in pdf.objects); cmyk_names=sum(_name_count(obj,pikepdf.Name("/DeviceCMYK"),set()) for obj in pdf.objects)
        intents=len(pdf.Root.get("/OutputIntents",[])); version=str(pdf.pdf_version); pdfx=str(pdf.docinfo.get("/GTS_PDFXVersion",""))
    bleed_expected=geometry["bleed_mm"]/25.4*72.0; trim=boxes["/TrimBox"]; media=boxes["/MediaBox"]
    ...
    checks={"pdf_version_at_least_1_6":tuple(int(x) for x in version.split(".")[:2])>=(1,6),"media_bleed_trim_art_boxes_present":all(len(boxes[x])==4 for x in ("/MediaBox","/BleedBox","/TrimBox","/ArtBox")),"bleed_at_least_requested":bleed_measured+0.05>=bleed_expected,"output_intent_present":intents>=1,"pdfx4_declared":pdfx=="PDF/X-4","device_rgb_absent":rgb_names==0,"device_cmyk_present":cmyk_names>0,"raster_images_min_300_ppi":images["all_at_least_300"],"fonts_embedded":fonts["all_embedded"]}
...
def export_prepress_package(master_svg: str | Path,output_dir: str | Path,*,proof_dpi:int=150,target_ppi:float=300.0,bleed_mm:float=3.0,trim_width_mm:float|None=None,trim_height_mm:float|None=None,icc_profile:str|Path|None=None) -> dict:
    ...
    inkscape=shutil.which("inkscape"); gs=shutil.which("gs"); qpdf=shutil.which("qpdf"); pdfcpu=shutil.which("pdfcpu"); pdfimages=shutil.which("pdfimages"); pdffonts=shutil.which("pdffonts")
    ...
    root,width_px,height_px=_canvas(master_svg); geometry=_trim_geometry(width_px,height_px,target_ppi=target_ppi,bleed_mm=bleed_mm,trim_width_mm=trim_width_mm,trim_height_mm=trim_height_mm)
    svg=svg_preflight(master_svg); source_has_raster=bool(svg["raster_images"])
    editable=output_dir/"artwork_editable.pdf"; press=output_dir/"artwork_press.pdf"; proof=output_dir/"artwork_proof.png"; commands=[]
    with tempfile.TemporaryDirectory(prefix="prepress-",dir=output_dir) as td:
        ...
        bleed_uri=_bleed_data_uri(master_svg,inkscape,geometry,temp,commands) if bleed_mm>0 else None
        _write_physical_svg(root,trim_svg,geometry,width_px,height_px); _write_physical_svg(root,press_svg,geometry,width_px,height_px,bleed_data_uri=bleed_uri)
        ...
        commands.append(_run([inkscape,press_svg,"--export-type=pdf",f"--export-dpi={target_ppi}",f"--export-filename={raw_press}"]))
    ...
    if pdfcpu:
        validations.append({"tool":"pdfcpu","mode":"relaxed-default",**_run([pdfcpu,"validate",press])}); diagnostics.append({"tool":"pdfcpu_strict","mode":"strict-diagnostic",**_run([pdfcpu,"validate","-mode","s",press])})
    pdf=_pdf_structural_report(press,geometry,pdfimages,pdffonts); tool_checks={v["tool"]:v["returncode"]==0 for v in validations}; diagnostic_checks={v["tool"]:v["returncode"]==0 for v in diagnostics}
    available={"inkscape":bool(inkscape),"ghostscript":bool(gs),"qpdf":bool(qpdf),"pdfcpu":bool(pdfcpu),"pdfimages":bool(pdfimages),"pdffonts":bool(pdffonts),"pikepdf":True}; required_tools=("ghostscript","qpdf","pdfcpu","pdfimages","pdffonts"); missing_validators=[name for name in required_tools if not available[name]]
    warnings=[]
    if not geometry["production_dimensions_confirmed"]: warnings.append("Trim size was not supplied; physical size was conservatively derived from the canvas at target PPI. Confirm production dimensions before manufacturing.")
    if source_has_raster and not geometry["source_meets_target_ppi"]: warnings.append(f"Photographic raster is only {min(geometry['effective_source_ppi_x'],geometry['effective_source_ppi_y']):.1f} PPI at requested trim size; press-ready status requires at least {target_ppi:.0f} PPI source detail.")
    ...
    source_resolution_ok=(not source_has_raster) or geometry["source_meets_target_ppi"]
    report={"schema":"poster-vector-preflight-v2","passed":technical_pass,"press_ready":technical_pass and geometry["production_dimensions_confirmed"] and source_resolution_ok,"svg":svg,"pdf":pdf,"geometry":geometry,"icc":{"profile":str(cmyk_icc),"profile_name":profile_name,"rgb_profile":str(rgb_icc) if rgb_icc else None},"tool_checks":tool_checks,"diagnostic_checks":diagnostic_checks,"tools":available,"missing_required_validators":missing_validators,"commands":commands,"validations":validations,"diagnostics":diagnostics,"warnings":warnings,"source_rules":{"target":"PDF/X-4-oriented","minimum_raster_ppi":300,"minimum_bleed_mm":3.0,"press_color_space":"CMYK","fonts":"embedded or outlined"},"pdfcpu_policy":"Use pdfcpu's official default relaxed validation for acceptance; retain strict mode as a non-blocking diagnostic.","press_pdf_policy":"CMYK PDF 1.6 with explicit MediaBox/BleedBox/TrimBox/ArtBox, ICC OutputIntent and a PDF/X-4 declaration. External PDF/X certification is not claimed without a dedicated PDF/X validator.","outputs":{"editable_pdf":str(editable),"press_pdf":str(press),"proof":str(proof),"report":str(output_dir/"preflight_report.json")}}
